...
void fsmjob::registration(ptt::fsmmessage* m)
{
...
void fsmjob::endregistration(ptt::fsmmessage* m)
{
  std::cout<<"Received "<<m->command()<<std::endl;
  std::cout<<"Received "<<m->value()<<std::endl;
...
void fsmjob::initialise(ptt::fsmmessage* m)
{
  std::cout<<"Received "<<m->command()<<std::endl;
  std::cout<<"Received "<<m->value()<<std::endl;
  ...
  Json::FastWriter f;
  std::stringstream sc;
  sc<<"JOBCONF="<<f.write(m_jconf);
  s_conf=sc.str();
 
}
void fsmjob::startProcess(ptt::processData* pProcessData)
{
  if (pProcessData->m_status != ptt::processData::NOT_CREATED)
    return;
  ...
  if (pid != 0)
    {
      ...
      pProcessData->m_status = ptt::processData::RUNNING;
      return;
    }
...
void fsmjob::killProcess(uint32_t pid, uint32_t sig)
{
	...
	{
	  if (iter->second->m_status == ptt::processData::RUNNING && pid == iter->first)
		{
		  PidToProcessMap::iterator toErase = iter;

		  ::kill(iter->first, sig);

		  iter->second->m_status = ptt::processData::KILLED;
		  delete iter->second;
...
void fsmjob::start(ptt::fsmmessage* m)
{
  ...
  for (Json::ValueConstIterator it = books.begin(); it != books.end(); ++it)
    {
      ...
      ptt::processData *pProcessData = new ptt::processData(jsonString);
      //std::cout<<" apres le process "<<std::endl;
      //getchar();
      pProcessData->m_processInfo["ENV"].append(s_conf);
...
void fsmjob::kill(ptt::fsmmessage* m)
{
...
void fsmjob::destroy(ptt::fsmmessage* m)
{
  ...
  if (pid!=0 && jss.compare("NONE")!=0)
    {
      ...
      ptt::processData *pProcessData = new ptt::processData(jss);
      this->startProcess(pProcessData);
      m_processMap.insert(PidToProcessMap::value_type(pProcessData->m_childPid, pProcessData));
      response["JOBS"]=this->jsonStatus();
      response["STATUS"]="DONE";
...
void fsmjob::registerjob(ptt::fsmmessage* m)
{
